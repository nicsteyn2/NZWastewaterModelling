...
st_date = as.Date("2023-01-01")
en_date = as.Date("2023-10-01")
...
warning("Processing of df_raw throws out a couple of extreme observations to make the plots nicer.")
df_raw = read.csv(paste0(filedir, filename, "_rawdata.csv")) %>%
  mutate(date = as.Date(date)) %>%
  filter(date >= st_date, date <= en_date) %>%
  filter(nGenomeCopies < 2e7) # Filter out a couple of very extreme values to make the figure prettier
...
CAR0 = 0.6078697
warning("Custom value of CAR0 being used")
...
plt_hiddenstates = ggplot(df %>% filter(variable %in% c("CARt", "Rt")), aes(x=date, y=mean, fill=variable)) +
  ...
  geom_hline(aes(yintercept=1), linetype=3, data=.%>%filter(variable=="Rt")) +
...
# ggsave("outputs/updates/fullseries.png", plt, dpi=300, width=30, height=15, units="cm")
ggsave("outputs/updates/2023only.png", plt, dpi=300, width=30, height=15, units="cm")
