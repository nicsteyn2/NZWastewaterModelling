...
# Load data
df_samples = loadCleanSamples(N_CHAINS, fname, MIN_ITER, THIN) %>% select(sigR, kw, iter, chain)
...
mcmc_list <- mcmc.list(
  lapply(split(df_samples, df_samples$chain), function(chain_df) {
    chain_mcmc = mcmc(as.matrix(chain_df[c("sigR", "kw")]))
    return(chain_mcmc)
  })
)

# Filter out estimated variables
mmhopts$paramNames = c("sigR", "kw")
mmhopts$proposalVariances = mmhopts$proposalVariances[c(2,4)]
...
produceResults = function() {
  ...
  if (TRACE_PLOTS) {
    ...
    df_samples_long = df_samples_nonthin %>% pivot_longer(cols=c(sigR, kw), names_to="param") #%>% filter(iter %in% keepiters)
