...
en_date = as.Date("2022-06-30")

# Load data
df = read.csv(paste0("outputs/filter/", filename, ".csv")) %>%
  mutate(date = as.Date(date)) %>%
  filter(date >= st_date, date <= en_date)

df_raw = read.csv(paste0("outputs/filter/", filename, "_rawdata.csv")) %>%
  mutate(date = as.Date(date)) %>%
  filter(date >= st_date, date <= en_date)
...
# Scale CAR (default t=51 )
CAR0 = df$mean[df$variable=="CARt" & df$date==as.Date("2022-03-31")]
...
plt_observeddata = ggplot(df %>% filter(variable %in% c("Cases", "Wastewater")), aes(x=date, y=mean)) +
  ...
  geom_ribbon(aes(ymin=lower, ymax=upper, fill = variable), alpha=0.6) +
