...
# This performs almost exactly the same calculations as covidParticleFilter.jl, ...
# ... except that it also calculates cumulative infections as an augmented hidden state.
# Since this is requires extra calculation we do not do this when running PMMH etc.
...
function covidParticleFilterWithCumulativeInfections(ρ, R, I, CI, Y, θ::Vector, opts)
    
    θ_matrix = repeat(θ', opts["tMax"], 1)
    return(covidParticleFilterWithCumulativeInfections(ρ, R, I, CI, Y, θ_matrix, opts))
    
end


# The primary particle filter!
function covidParticleFilterWithCumulativeInfections(ρ, R, I, CI, Y, θ::Matrix, opts)
    ...
    for tt = (opts["windinPeriod"]+1):opts["tMax"]
        ...
        try
            resampleParticlesWithCumulativeInfections!(tt, W, ρ, R, I, CI, θ[tt,:], opts)
        catch e
            ...
            return(NaN, NaN, NaN, NaN, zeros(opts["Nx"]))
        end
...
function initialiseHiddenStatesWithCumulativeInfections(Y, opts)
    
    # Initialise hidden-states
    ρ = zeros(opts["Nx"], opts["tMax"])
    R = zeros(opts["Nx"], opts["tMax"])
    I = zeros(opts["Nx"], opts["tMax"])
    CI = zeros(opts["Nx"], opts["tMax"])
...
function resampleParticlesWithCumulativeInfections!(tt, W, ρ, R, I, CI, θ, opts)
    ...
    CI[:,st_time:tt] = CI[inds,st_time:tt]
